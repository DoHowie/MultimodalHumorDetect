...
def load_pickle_data(pkls_dir,filter_json={}):
    ...
    for filename in tqdm(pkl_files,desc='loading pickles',disable=True):
        ...
        with open(file_path,'rb') as f:
            ...
            id = filename.split('.')[0]
            resulting_dict[id] = torch.from_numpy(obj)
    gc.enable()
...
def validation(model,dataloader,loss_fn):
    ...
    with torch.no_grad():
        for data in tqdm(dataloader,desc='Training...',total=len(dataloader),disable=True):
            ...
            val_running_loss += loss.clone().detach()
            val_running_acc += accuracy(outputs.clone().detach(),labels)

    val_loss = val_running_loss/len(dataloader)
    val_acc = val_running_acc/len(dataloader)
...
def main():
    ...
    acoustic_feature_pkls_dir = '../../../../../scratch1/jiaqilu/CSCI535/CSCI535-Project/dataset/urfunny2_visual_feature_pkl/'
    ...
    acousticmodel = AcousticFeatureLSTMClassifier(hidden_dim=768,lstm_hidden=128,bidirectional=True).to('cuda')
    epochs = args.epochs
    lr = args.lr
    ...
    for epoch in range(1,epochs+1):
        ...
        val_epoch_loss, val_epoch_acc = validation(acousticmodel,test_dataloader,loss_function)
